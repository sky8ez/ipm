...
.controller('printController', function($scope, $routeParams, $mdDialog,$window, Forms) {
    ...
    $scope.onchange = function() {// change template
      //get table list ajax
      Forms.get('api/print-get-row-count/' + $scope.form_name  +  '/' + $scope.form_id + '/' + $scope.cmbtemplate)
        .success(function(data) {
          $scope.row_per_page = data[1]; //get row per page
          var pages = [];
          var j = 1;
          for(var i = 0; i < data[0]; i += parseInt($scope.row_per_page)) {
            pages.push(j);
            j = j + 1;
            console.log(i);
          }
          $scope.loop = pages;
         if ($scope.loop.length == 0) { //jika tidak ada row, tetap harus print 1 lembar
           pages.push(1);
           $scope.loop = pages;
         }
        })
      ...
      Forms.get('api/print/' + $scope.form_name  +  '/' + $scope.form_id + '/' + $scope.cmbtemplate)
        .success(function(data) {
                        ...
                        //  console.log(arr);
                          ...
                          switch ($scope.print_details[i]['type']) {
                            case 'textbox':
                               ...
                               if (p_value == '#page_number') {
                                 for (var k=1;k<=$scope.loop.length;k++) {
                                   createTextbox(p_kind,".print_form"+k, p_id, p_top,p_left,
                                           p_width,p_height,
                                           p_horizontal_align,p_vertical_align,
                                           p_border_flag,p_border_width,p_border_color,p_border_style,
                                           p_line_height,p_padding_x,p_padding_y,p_font_weight,p_font_style,
                                           p_font_size,p_font_color,p_font_family,p_use_parent_flag,
                                           'page ' + k + ' of ' + $scope.loop.length , p_value_type, p_value_format);
                                 }
            ...
            $('.table1 thead').append(row);//thead untuk header
            ...
            $.each($scope.details, function(key, value) {
              ...
              row += "</tr>"
              $('.tab' + tablecount + ' tbody').append(row);
            });


            if (count < $scope.row_per_page) {
              for(var i=0 ; i< ($scope.row_per_page - count);i++) {
                var row = "<tr style='height:{{$print->row_height}}'>";
                for(var j=0;j<column_count;j++) {
                  row += "<td style='padding:2px;vertical-align:top;text-align:" + $('.table1 thead tr').eq(0).find('th').eq(i).attr('textalign') + ";font-size:" + $('.table1 thead tr').eq(0).find('th').eq(i).attr('fontsize') + "'>&nbsp;</td>";
                }
                row += "</tr>"
                $('.tab' + tablecount + ' tbody').append(row);
              }
