  ...
  public function __construct(ValidatorRepository $validator,AccessRepository $access)
 {
  ...
  public function index($skip = 0)
  {
    $this->access_list = $this->access->getAccess($this->form_id, Session::get('user_access_id'));
      ...
      $headers = [];
      $header = ['label' => trans('password.type'),'value' => 'mt_password_word.type', 'type' => 'text', 'table' => '', 'sort' => 'sort'];
      array_push($headers,$header);
      $header = ['label' => trans('password.word'),'value' => 'mt_password_word.word', 'type' => 'text', 'table' => '', 'sort' => 'sort'];
      array_push($headers,$header);
      ...
      $records = Mt_password_word::whereRaw($search)
      ...
      if ($skip == 0) {
        $records_count = Mt_password_word::whereRaw($search)
        ->where('delete_flag',false)
        ->count();
      } else {
        $records_count = 0;
      ...
      foreach ($records as $record  ) {
        $row = ['id' => $record->id,
                 'records' => [
                             ...
                             ['text_align' => 'left', 'type' => 'text' , 'value' => $record->word],
