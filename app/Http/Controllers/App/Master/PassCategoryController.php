...
class PassCategoryController extends Controller
{
   ...
   * @return Response
   */
  private $form_id = "PASSWORD-CATEGORY";
  ...
  public function __construct(ValidatorRepository $validator,AccessRepository $access)
 {
  ...
  public function index($skip = 0)
  {
    $this->access_list = $this->access->getAccess($this->form_id, Session::get('user_access_id'));
      ...
      array_push($headers,$header);
      $header = ['label' => trans('password_category.updated_at'),'value' => 'mt_password_category.updated_at', 'type' => 'text', 'table' => '', 'sort' => 'sort'];
      array_push($headers,$header);
      ...
      $records = Mt_password_category::whereRaw($search)
      ...
      if ($skip == 0) {
        $records_count = Mt_password_category::whereRaw($search)
        ->where('delete_flag',false)
        ->count();
      } else {
        $records_count = 0;
  ...
  public function getForm($cond="insert", $id = "")
  {
    ...
    $data = [];
    if ($id != "") {
      $data = Mt_password_category::where('mt_password_category.id',$id)
      ->first(['mt_password_category.*']);
    }
    ...
    $form = ['label' => 'Name','placeholder' => 'Name',
             ...
             'required' => 'true',
             'unique' => 'unique:mt_password_category,name,'.$id,
  ...
  public function store(Request $request)
  {
       ...
       if ($validator->passes()) {
         ...
         DB::beginTransaction();
         try {
           $record = Mt_password_category::create($save_datas);

           $audit = new Tr_audit;
           $audit->transaction_category = 'password-category';
           ...
           DB::commit();
           $arr = array('code' => "200", 'status' => "OK", 'url' => '#/list/password-category');
  ...
  public function update(Request $request, $id)
  {
       ...
       if ($validator->passes()) {
         ...
         DB::beginTransaction();
         try {
           Mt_password_category::where('id',$id)
                    ->update($save_datas);
  ...
  {
     $record = Mt_password_category::find($id);
     if(isset($record)) {
  ...
  public function destroyMany(Request $request)
  {
    $datas = $request->data;
     ...
     try {
       foreach ($datas as $data) {
         $record = Mt_password_category::find($data);
         if(isset($record)) {
            $record->delete_flag = true;
            $record->update();
         }
