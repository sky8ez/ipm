...
class SSPNEW {
    ...
    static function order ( $request, $columns, $isJoin = false )
    {
        ...
        if ( isset($request['order']) && count($request['order']) ) {
            $orderBy = array();
            $dtColumns = SSPNEW::pluck( $columns, 'dt' );
        ...
        if ( isset($request['search']) && $request['search']['value'] != '' ) {
            ...
            for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
                ...
                if ( $requestColumn['searchable'] == 'true' ) {
                    $binding = SSPNEW::bind( $bindings, '%'.$str.'%', PDO::PARAM_STR );
    ...
    {
        $bindings = array();
        $db = self::db( self::connection() );

        // Build the SQL query string from the request
        $limit = SSPNEW::limit( $request, $columns );
        $order = SSPNEW::order( $request, $columns, $joinQuery );
        $where = SSPNEW::filter( $request, $columns, $bindings, $joinQuery );
        ...
        if($joinQuery){
            ...
            $col = SSPNEW::pluck($columns, 'db', $joinQuery);
            ...
            $query =  "SELECT SQL_CALC_FOUND_ROWS `".implode("`, `", SSPNEW::pluck($columns, 'db'))."`
			 FROM `$table`
			 $where
			 $extraWhere
			 $groupBy
        ...
        $data = SSPNEW::sql_exec( $db, $bindings,$query);

        // Data set length after filtering
        $resFilterLength = SSPNEW::sql_exec( $db,
            "SELECT FOUND_ROWS()"
        );
        $recordsFiltered = $resFilterLength[0][0];

         // Total data set length
        $count_request = "SELECT COUNT({$table}.{$primaryKey})";
        if($joinQuery){
          $count_request .= $joinQuery;
        } else {
          $count_request .= "FROM   `$table`";
        }
        $resTotalLength = SSPNEW::sql_exec( $db,$count_request);
        ...
        return array(
            ...
            "data"            => SSPNEW::data_output( $columns, $data, $joinQuery )
        );
    }
    ...
    static function sql_exec ( $db, $bindings, $sql=null )
    {
        ...
        try {
          //  echo $sql;
            ...
            SSPNEW::fatal( "An SQL error occurred: ".$e->getMessage() );
        }
